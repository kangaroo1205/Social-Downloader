...
- 檔案自動以「年份_月份_日期_序號」命名整理，並集中存在專屬資料夾 (如 `threads@username`) 中。
...
或者直接執行 `python threads_scraper.py`，程式會提示你輸入網址。
...
## 輸出路徑
所有下載的媒體檔案將被儲存至當前目錄下的 `threads@<username>` 資料夾中，每個檔案的命名格式為 `YYYY_MM_DD_N.ext`。

## 注意事項
- 請勿將此工具用於非法用途或侵犯他人版權。
- 若 Threads 官方更新了網頁架構或 API 格式，此工具可能需要相應更新才能繼續正常運作。
