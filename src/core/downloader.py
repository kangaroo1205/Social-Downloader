...
async def download_file(
    ...
    if os.path.exists(dest_path):
        logger.info(f"   ⏭  已存在，跳過：{os.path.basename(dest_path)}")
        return True
    ...
    async with semaphore:
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                logger.success(f"   ✅ 下載完成：{os.path.basename(dest_path)}")
                return True
            except Exception as e:
                if attempt < MAX_RETRIES:
                    logger.warning(f"   ⚠️  下載失敗（第 {attempt} 次），重試中：{os.path.basename(dest_path)} — {e}")
                    await asyncio.sleep(2 ** attempt)
                else:
                    logger.error(f"   ❌ 下載失敗（已達最大重試次數）：{os.path.basename(dest_path)} — {e}")
    return False
