...
async def scrape_post(post_url: str) -> list[dict]:
    ...
    if use_session:
        logger.info(f"🔑 偵測到 X.com 登入 session（{COOKIES_FILE}），將以登入狀態爬取")
    else:
        logger.warning("   ⚠️ 未偵測到登入 session，X.com 未登入可能無法查看任何內容（建議執行 --login-x 登入）")
    ...
    logger.info(f"📦 共攔截到 {len(all_json_blobs)} 個 GraphQL 回應，開始解析...")
...
async def do_login():
    ...
    logger.info("🔐 開啟瀏覽器，請在瀏覽器中完成 X (Twitter) 登入...")
    ...
    async with async_playwright() as pw:
        ...
        logger.info("   等待登入完成（偵測到首頁網址後自動儲存）...")
        try:
    ...
    logger.success(f"✅ X.com 登入 session 已儲存至：{COOKIES_FILE}\n")
...
async def scrape_profile(profile_url: str) -> list[dict]:
    ...
    async with async_playwright() as pw:
        ...
        if "Something went wrong" in page_text or "出錯了" in page_text or "Retry" in page_text:
            logger.warning("⚠️  頁面似乎出現錯誤，可能被限制或需登入。")

        logger.info("📜 開始向下滾動載入推文...")
        no_change_count = 0
    ...
    logger.success(f"✅ 共找到 {len(seen_post_ids)} 篇不重複推文，{len(all_media)} 個媒體檔案")
    return all_media
