...
async def scrape_post(post_url: str) -> list[dict]:
    ...
    if use_session:
        logger.info(f"🔑 偵測到 Instagram 登入 session（{COOKIES_FILE}），將以登入狀態爬取")
    else:
        logger.info("   未偵測到登入 session，以訪客模式爬取")
        logger.warning("   ⚠️  Instagram 對未登入訪客限制嚴格，建議先執行 --login-instagram")

    logger.info(f"🌐 開啟瀏覽器，載入貼文：{post_url}")
    ...
    async with async_playwright() as pw:
        ...
        try:
            ...
            logger.warning(f"⚠️  頁面載入警告（將繼續）：{e}")

        await asyncio.sleep(2)
        await browser.close()

    logger.info(f"📦 共收集到 {len(all_json_blobs)} 個含媒體資料的 JSON 片段，開始解析...")
    seen_ids: set = set()
    all_media: list[dict] = []
    ...
    logger.success(f"✅ 找到 {len(all_media)} 個媒體檔案")
    return all_media
...
async def do_login():
    ...
    logger.info("🔐 開啟瀏覽器，請在瀏覽器中完成 Instagram 登入...")
    logger.info("   登入完成後程式會自動偵測並儲存 session，請勿手動關閉瀏覽器。")
    ...
    async with async_playwright() as pw:
        ...
        logger.info("   等待登入完成（偵測到已登入後自動儲存）...")
        try:
            # 等待直到 URL 不再包含 login
            ...
            logger.warning("⚠️  等待逾時，嘗試直接儲存目前 session...")
        await asyncio.sleep(2)
        await context.storage_state(path=COOKIES_FILE)
        await browser.close()

    logger.success(f"✅ Instagram 登入 session 已儲存至：{COOKIES_FILE}\n")
...
async def scrape_profile(profile_url: str) -> list[dict]:
    ...
    logger.info(f"🌐 開啟瀏覽器，載入頁面：{profile_url}")
    ...
    async with async_playwright() as pw:
        ...
        if any(kw in page_text for kw in [
            ...
            logger.error("❌ 此帳號為私人帳號或不存在，無法爬取內容。")
            await browser.close()
            return []

        # Instagram 需要滾動讓 API 持續回傳更多貼文
        logger.info("📜 開始向下滾動載入所有貼文...")
        no_change_count = 0
        ...
        while no_change_count < MAX_NO_CHANGE:
            ...
            if new_height == last_height:
                no_change_count += 1
                logger.info(f"   頁面高度未變化（{no_change_count}/{MAX_NO_CHANGE}）")
            else:
                no_change_count = 0
                last_height = new_height
                logger.info(f"   已滾動，頁面高度：{new_height}px")
    ...
    logger.success(f"✅ 共找到 {len(seen_ids)} 篇不重複貼文，{len(all_media)} 個媒體檔案")
    return all_media
