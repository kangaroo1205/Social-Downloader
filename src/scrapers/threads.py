...
async def scrape_post(post_url: str) -> list[dict]:
    ...
    if use_session:
        logger.info(f"🔑 偵測到 Threads 登入 session（{COOKIES_FILE}），將以登入狀態爬取")
    else:
        logger.info("   未偵測到登入 session，以訪客模式爬取（可執行 --login-threads 登入）")
    ...
    logger.info(f"📦 共收集到 {len(all_json_blobs)} 個含貼文資料的 JSON 片段，開始解析...")
...
async def do_login():
    ...
    logger.info("🔐 開啟瀏覽器，請在瀏覽器中完成 Threads 登入...")
    ...
    logger.success(f"✅ Threads 登入 session 已儲存至：{COOKIES_FILE}\n")
...
async def scrape_profile(profile_url: str) -> list[dict]:
    ...
    logger.success(f"✅ 共找到 {len(seen_post_ids)} 篇不重複貼文，{len(all_media)} 個媒體檔案")
    return all_media
